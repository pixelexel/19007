...
render() {
  ...
  const { pp_data,ss_no,ex_curr,sport_d,top_marks,top_sport,top_extra_curr,t_s_a,t_s_s,t_s_e,p_c,p_b,p_g,schools} = this.props.district
      ...
      <Grid container style={{margin: 25, maxWidth: 'calc(100% - 50px)'}}>
        ...
        <Grid container style={{margin: 25, maxWidth: 'calc(100% - 50px)'}}>
          ...
          <Paper  style={{height:'350px'}}>
            <Typography style={{fontSize:'25px',textAlign:'center'}}>School List</Typography>
<div className={classes.root2}>
                      <Table>
                      <TableBody>
                        { schools.map((d) => {
                       return <TableRow>
                          <TableCell onClick={() => {this.props.dispatch(changeScreen(screens.SCHOOL, d))}}>{d}</TableCell>
                      </TableRow>
             })
             }</TableBody></Table></div>
          </Paper>
